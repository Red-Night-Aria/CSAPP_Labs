...
//volatile sig_atomic_t reaped_pid;
    ...
    if (strcmp(argv[0], "quit") == 0){
        ...
        if (jobPtr){
            ...
            if (strcmp(argv[0], "bg") == 0)
                jobPtr->state = BG;
            else jobPtr->state = FG;
            kill(-1*getpgid(jobPtr->pid), SIGCONT);
            if (jobPtr->state == FG)
                waitfg(jobPtr->pid);
            sigprocmask(SIG_SETMASK, &prev_one, NULL);
        }
...
void waitfg(pid_t pid)
{
    struct job_t* fgPtr = getjobpid(jobs, pid);
    if (fgPtr == NULL || fgPtr->state != FG){
        printf("pid %d isn't a frontground job!\n", pid);
    }
    while (fgPtr->state == FG){
        sigsuspend(&prev_one);
    }
    return;
}
...
void sigchld_handler(int sig) 
{
    ...
    while ((reaped_pid=waitpid(-1, NULL, WNOHANG))>0){
        //printf("reap pid: %d\n", reaped_pid);
        if (deletejob(jobs, reaped_pid) == 0){
            printf("delete error\n");
        }
    }
    return;
...
void sigtstp_handler(int sig) 
{
    ...
    sigprocmask(SIG_BLOCK, &mask_one, &prev_one);
    struct job_t* fgPtr = getjobpid(jobs, fg);
